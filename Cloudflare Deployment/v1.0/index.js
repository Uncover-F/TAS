...
function temporarilyBanIP(ip, duration = 15000) {
  if (typeof ip === 'string') {
...
export default {
  async fetch(request, env) {
    ...
    if (temporaryBanTracker[clientIP] && temporaryBanTracker[clientIP] > currentTime) {
      sendToDiscord(`Temporary ban still active for IP: ${clientIP}`);
      await sleep(1500);
    ...
    if (requestTracker[clientIP] && requestTracker[clientIP] > requestTime - 100) {
      sendToDiscord(`Rate limit exceeded for IP: ${clientIP}`);
      temporarilyBanIP(clientIP); // Temporarily ban for 15 seconds
      await sleep(1500);
      return new Response(JSON.stringify({ error: 'Rate limit exceeded. Try again later.' }), {
        status: 429,
        headers: { 'Content-Type': 'application/json' },
